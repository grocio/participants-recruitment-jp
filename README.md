...
スプレッドシートのタイムゾーン設定は初期状態ではグリニッジ標準時になっています（しかし，設定を開くと「東京時間」に設定されていることになっています。たぶんバグ[2019年5月現在]）。
このままだとメールに表示される時間がおかしくなるので，スプレッドシートのタイムゾーン設定を日本時間と同等の時間に変更します。
下図を参考に，スプレッドシートのタイムゾーンをソウルに設定してください。
日本とタイムゾーン設定が同じなので，これで大丈夫です（気になる人は変更の保存後，東京に戻してもバグは起きません）。

![](images/changeTimezone.gif)
...
ただし，<u>**予約完了のトリガーとして使用されるものについては必ず半角数字列にしてください**</u>
